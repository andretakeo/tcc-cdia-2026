...
**Granularidade temporal e risco de look-ahead.** Os artigos coletados possuem *timestamps* completos em formato ISO 8601 (e.g., `2026-03-16T10:56:56`), mas a etapa de agregação diária descarta o componente de hora ao extrair apenas os dez primeiros caracteres da *string* de data (`df["date"] = a["date"][:10]`). Como consequência, todos os artigos publicados em um mesmo dia calendário — incluindo aqueles publicados após o fechamento do pregão da B3 (aproximadamente às 17h00 BRT) — são atribuídos ao vetor de sentimento desse mesmo dia t e alinhados com os dados OHLCV de t. Se o efeito de preço de uma notícia publicada após o fechamento só se materializa na abertura do dia t+1, esse alinhamento introduz um risco residual de *look-ahead* de um dia útil.

Três fatores mitigam a relevância prática desse risco no contexto do presente estudo. Primeiro, o horizonte de previsão adotado é de 5 a 21 dias úteis: um deslocamento de um dia representa, no pior caso, menos de 5% do horizonte mínimo, tornando improvável que ele altere a direção da variável-alvo. Segundo, a agregação por média diária dilui o peso de artigos individuais publicados em horários específicos, reduzindo a influência de qualquer notícia pós-fechamento sobre o vetor resultante. Terceiro, o *forward-fill* propaga o último sentimento observado para frente no tempo — operando na direção causal correta — e não introduz informação futura sobre dias ainda não ocorridos. Note-se que o deslocamento pode exceder um dia quando há dias consecutivos sem publicação: se não há notícia nos dias $t+1$, $t+2$ e $t+3$, o sentimento do dia $t$ preenche todos os três, amplificando a defasagem. A magnitude típica deste efeito depende da frequência de publicação do InfoMoney para cada ativo. Para estudos com horizonte intradiário ou h = 1 dia útil, entretanto, seria necessário filtrar explicitamente os artigos pelo horário de publicação, retendo apenas aqueles publicados antes do fechamento do pregão para a construção do vetor de sentimento de cada dia.
